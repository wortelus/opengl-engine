...
void ShaderUniforms::passEvent(const EventArgs &event_args) {
    switch (event_args.type) {
        ...
        case EventType::U_VIEW_MATRIX: {
            const auto *view_matrix = static_cast<const EventPayload<glm::mat4> *>(&event_args);
            view.value = view_matrix->getPayload();
            view.is_dirty = true;
            break;
        }
        case EventType::U_PROJECTION_MATRIX: {
            const auto *projection_matrix = static_cast<const EventPayload<glm::mat4> *>(&event_args);
        ...
        case EventType::U_CAMERA_POSITION: {
            const auto *camera_pos = static_cast<const EventPayload<glm::vec3> *>(&event_args);
